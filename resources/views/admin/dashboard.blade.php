            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-8 py-6">
                            <p class="font-bold uppercase tracking-wide text-sm truncate max-w-[150px]">{{ $trx->name ?? $trx->customer_name }}</p>
                            <p class="text-xs text-slate-400 truncate max-w-[150px]">{{ $trx->email ?? $trx->customer_email }}</p>
                        </td>
                        <td class="px-8 py-6 font-medium text-slate-600 max-w-xs truncate">
                            {{ $trx->event->title ?? 'Event Terhapus' }}
                        </td>
                        ...
                        <td class="px-8 py-6 font-black text-indigo-600 whitespace-nowrap text-right">
                            @if(($trx->total_price ?? $trx->total_amount) == 0)
                                Gratis
                            @else
                                Rp {{ number_format($trx->total_price ?? $trx->total_amount, 0, ',', '.') }}
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-8 py-10 text-center text-slate-500 font-medium">Belum ada transaksi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
