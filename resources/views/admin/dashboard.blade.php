...
<main class="flex-1 p-10 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <h3 class="text-2xl font-black">{{ $activeEventsCount ?? 0 }} Event</h3>
            </div>
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <h3 class="text-2xl font-black">{{ $pendingOrdersCount ?? 0 }} Pesanan</h3>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y border-t">
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            <td class="px-8 py-6">
                                <p class="font-bold uppercase tracking-wide text-sm">{{ $transaction->name }}</p>
                                <p class="text-xs text-slate-400">{{ $transaction->email }}</p>
                            </td>
                            <td class="px-8 py-6 font-medium text-slate-600">{{ $transaction->event->title ?? 'Event Terhapus' }}</td>
                            <td class="px-8 py-6">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-8 py-6 text-center text-slate-500 font-medium">Belum ada transaksi.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
