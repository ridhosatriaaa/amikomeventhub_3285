...
{{-- Mengambil nama langsung dari URL parameter (?name=...) --}}
<p class="font-bold text-lg">{{ request('name') ?? 'Pemesan' }}</p>
                    <div>
                        <p class="text-slate-400 text-xs font-bold uppercase mb-1">Tanggal</p>
                        {{-- Tanggal Dinamis --}}
                        <p class="font-bold text-lg">{{ \Carbon\Carbon::parse($event->date)->format('d M Y') }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        {{-- Lokasi Dinamis --}}
                        <p class="font-bold">{{ $event->location ?? 'Amikom Yogyakarta' }}</p>
                    </div>
                </div>
                ...
                <div class="bg-slate-100 p-6 rounded-3xl flex flex-col items-center">
                    ...
                    <p class="mt-4 font-mono font-bold text-slate-800">TKT-{{ rand(100000000, 999999999) }}</p>
                </div>
            </div>
